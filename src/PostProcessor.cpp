...
PostProcessor::PostProcessor(Shader shader, GLuint width, GLuint height)
    ...
    // Initialize render buffer / frame buffer object
    glGenFramebuffers(1, &this->MSFBO);
    glGenFramebuffers(1, &this->FBO);
    glGenRenderbuffers(1, &this->RBO);

    // Initialize render buffer storage with a multisampled color buffer (don't need depth/stencil buffer)
    ...
    // Also initialize the FBO to blit multisampled color buffer to;
    ...
    this->InitRenderData();
    this->PostProcessorShader.Use().SetInt("scene", 0);
...
void PostProcessor::Render(GLfloat dt) {
    ...
    this->PostProcessorShader.SetFloat("time", dt);
    this->PostProcessorShader.SetBool("confuse", this->Confuse);
    this->PostProcessorShader.SetBool("chaos", this->Chaos);
    this->PostProcessorShader.SetBool("shake", this->Shake);
